...
export default function EditCropPage() {
  ...
  const [cropData, setCropData] = useState<any>(null); // Store crop data
  ...
  useEffect(() => {
    if (id) {
      const fetchCrop = async () => {
        try {
          // Reference the specific crop document in the Firestore collection
          const cropDocRef = doc(db, "crops", id as string); // Correct the reference using 'id'
          const cropDocSnap = await getDoc(cropDocRef);

          if (cropDocSnap.exists()) {
            setCropData(cropDocSnap.data()); // Set the crop data if the document exists
          } else {
            setErrorMessage("Crop not found");
          }
        } catch (error) {
          setErrorMessage("Error fetching crop data");
        } finally {
          setLoading(false);
        }
      };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Handle form submission to update the crop data
      const cropDocRef = doc(db, "crops", id as string);
      await setDoc(cropDocRef, cropData); // Update the document in Firestore with the new data
      router.push("/crops"); // Redirect back to the crops page
    } catch (error) {
      setErrorMessage("Error updating crop data");
    }
  };

  if (loading) return <p>Loading crop data...</p>;
