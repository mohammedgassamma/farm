...
export const LivestockClientScreen = () => {
  ...
  const handleExport = () => {
    ...
    try {
      ...
      exportToExcel({
        data: dataToExport,
        fileName: `elevage_${new Date().toISOString().split("T")[0]}`,
        sheetName: "Elevage",
      });
        ...
        >
          ...
          <div className="mb-6 bgcards p-6 rounded-md">
            ...
            <p className="font-semibold text-lg">
              ...
              {formatCurrency({ number: livestockData?.totalRevenue || 0 })}
            </span>
            </p>
            <p className="font-semibold text-lg">
              {t("cost")}:{" "}
              <span className="text-red-500">
              {formatCurrency({ number: livestockData?.totalExpenses || 0 })}
            </span>
            </p>
            <p className="font-semibold text-lg">
              {t("profitAndLoss")}:{" "}
              {formatCurrency({ number: livestockData?.totalProfit || 0 })}
            </p>
          </div>
      ...
      >
        <div className="flex gap-[1rem] items-start w-full border-b mb-4 pb-2">
          ...
          <div>
            <Button
                onClick={(e) => {
                  e.stopPropagation();
                  handleDeleteConfirmation(animal.id);
                }}
          ...
          <div className="flex items-center space-x-4 relative">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-sm text-gray-500">
                {t("income")}:{" "}
                <span className="text-green-500">
                {formatCurrency({ number: animal.totalIncome })}
              </span>
              </p>
              <p className="text-sm text-gray-500">
                {t("cost")}:{" "}
                <span className="text-red-500">
                {formatCurrency({ number: animal.totalCost })}
              </span>
              </p>
            </div>
          </div>
        ...
        <div className="w-full">
          <p className="text-xl font-bold text-right">
            {t("profitAndLoss")}:{" "}
            {formatCurrency({ number: animal.totalProfit })}
          </p>
        </div>
